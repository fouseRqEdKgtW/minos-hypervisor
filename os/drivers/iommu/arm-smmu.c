...
struct arm_smmu_domain {
	...
	mutex_t				init_mutex; /* Protects smmu pointer */
...
static void parse_driver_options(struct arm_smmu_device *smmu)
{
	...
	do {
		if (of_get_bool(smmu->node, arm_smmu_options[i].prop)) {
			smmu->options |= arm_smmu_options[i].opt;
			pr_info("arm-smmu: option %s\n", arm_smmu_options[i].prop);
		}
...
static vmm_irq_return_t arm_smmu_context_fault(int irq, void *dev)
{
	...
	pr_err("Unhandled context fault: fsr=0x%x, iova=0x%08lx, fsynr=0x%x, cb=%d\n",
...
static vmm_irq_return_t arm_smmu_global_fault(int irq, void *dev)
{
	...
	pr_err_once("Unexpected global fault, this could be serious\n");
	pr_err_once("\tGFSR 0x%08x, GFSYNR0 0x%08x, GFSYNR1 0x%08x, GFSYNR2 0x%08x\n",
					...
					struct arm_smmu_device *smmu)
{
	...
	if (ret < 0) {
		pr_err("failed to request context IRQ %d (%u)\n",
			   cfg->irptndx, irq);
...
static phy_addr_t arm_smmu_iova_to_phys_hard(struct vmm_iommu_domain *domain,
						  phy_addr_t iova)
{
	struct arm_smmu_domain *smmu_domain = to_smmu_domain(domain);
	struct arm_smmu_device *smmu = smmu_domain->smmu;
	...
	try = 100;
	tmp = readl_relaxed(cb_base + ARM_SMMU_CB_ATSR);
	...
	if ((tmp & ATSR_ACTIVE) && !try) {
		pr_err("iova to phys timed out. Falling back to software table walk.\n");
	...
	vmm_spin_unlock_irqrestore(&smmu_domain->cb_lock, flags);
	if (phys & CB_PAR_F) {
		pr_err("translation fault!\n");
		pr_err("PAR = 0x%"PRIPADDR"\n", phys);
		return 0;
	}
...
static phy_addr_t arm_smmu_iova_to_phys(struct vmm_iommu_domain *domain,
					     phy_addr_t iova)
{
	struct arm_smmu_domain *smmu_domain = to_smmu_domain(domain);
	struct io_pgtable_ops *ops = smmu_domain->pgtbl_ops;
...
static int arm_smmu_map(struct vmm_iommu_domain *domain, phy_addr_t iova,
					...
					struct vmm_iommu_controller *ctrl)
{
	...
	smmu_domain = zalloc(sizeof(*smmu_domain));
	if (!smmu_domain)
		return NULL;
	...
	if (ret) {
		pr_err("Failed to init SMMU context bank (error %d)\n",
			   ret);
		free(smmu_domain);
		return NULL;
	}
				  ...
				  struct vmm_device *dev)
{
	...
	pr_info("arm-smmu: attached %s device to domain=0x%p\n",
			dev->name, domain);
				   ...
				   struct vmm_device *dev)
{
	...
	pr_info("arm-smmu: detached %s device from domain=0x%p\n",
			dev->name, domain);
...
static int arm_smmu_add_device(struct vmm_device *dev)
{
	...
	if (dev->iommu_priv) {
		pr_err("%s: IOMMU driver already assigned to device\n",
			__func__);
		return -EINVAL;
	}

	archdata = zalloc(sizeof(*archdata));
	if (!archdata) {
		return -ENOMEM;
	}

	sids = zalloc(num_sid * sizeof(*sids));
	if (!sids) {
		ret = -ENOMEM;
	...
	if (VMM_IS_ERR(group)) {
		pr_err("Failed to allocate IOMMU group\n");
	...
	if (ret < 0) {
		pr_err("Failed to add device to IOMMU group\n");
	...
	pr_info(smmu->node->name, "arm-smmu: added %s device\n", dev->name);
...
static void arm_smmu_remove_device(struct vmm_device *dev)
{
	...
	pr_info("arm-smmu: removed %s device\n", dev->name);
...
{
	unsigned long size;
	struct device_node *node = smmu->node;
	void *gr0_base = ARM_SMMU_GR0(smmu);
	u32 id;
	...
	if (id & ID0_S1TS) {
		...
		pr_info("arm-smmu: stage 1 translation\n");
	}

	if (id & ID0_S2TS) {
		smmu->features |= ARM_SMMU_FEAT_TRANS_S2;
		pr_info("arm-smmu: stage 2 translation\n");
	}

	if (id & ID0_NTS) {
		smmu->features |= ARM_SMMU_FEAT_TRANS_NESTED;
		pr_info("arm-smmu: nested translation\n");
	}

	if (!(smmu->features &
		(ARM_SMMU_FEAT_TRANS_S1 | ARM_SMMU_FEAT_TRANS_S2))) {
		pr_err("%s: no translation support!\n", __func__);
		return -ENODEV;
	}
	...
	if ((id & ID0_S1TS) &&
		...
		pr_info("arm-smmu: address translation ops\n");
	}
	...
	if (cttw_fw || cttw_reg)
		pr_info("arm-smmu: %scoherent table walk\n",
			  cttw_fw ? "" : "non-");
	if (cttw_fw != cttw_reg)
		pr_info("arm-smmu: (IDR0.CTTW overridden by "
			  "FW configuration)\n");
	...
	if (id & ID0_SMS) {
		...
		if (size == 0) {
			pr_err("%s: stream-matching supported, "
				   "but no SMRs present!\n", __func__);
			return -ENODEV;
		}
	} else {
	...
	pr_info("arm-smmu: num_groups=%d streamid_mask=0x%x\n",
	...
	if (smmu->num_s2_context_banks > smmu->num_context_banks) {
		pr_err("%s: impossible number of S2 context banks!\n", __func__);
		return -ENODEV;
	}
	pr_info("arm-smmu: %u context banks (%u stage-2 only)\n",
	...
	if (smmu->model == CAVIUM_SMMUV2) {
		...
		pr_info("arm-smmu: enabling workaround for Cavium erratum 27704\n");
	}
	...
	if (arm_smmu_ops.pgsize_bitmap == -1UL)
		...
		arm_smmu_ops.pgsize_bitmap |= smmu->pgsize_bitmap;
	pr_info("arm-smmu: Supported page sizes: 0x%08lx\n",
		  smmu->pgsize_bitmap);

	if (smmu->features & ARM_SMMU_FEAT_TRANS_S1)
		pr_info("arm-smmu: Stage-1: %lu-bit VA -> %lu-bit IPA\n",
			  smmu->va_size, smmu->ipa_size);

	if (smmu->features & ARM_SMMU_FEAT_TRANS_S2)
		pr_info("arm-smmu: Stage-2: %lu-bit IPA -> %lu-bit PA\n",
...
static void arm_smmu_device_reset(struct arm_smmu_device *smmu)
{
	...
	/* Push the button */
	arm_smmu_tlb_sync_global(smmu);
	writel_relaxed(reg, ARM_SMMU_GR0_NS(smmu) + ARM_SMMU_GR0_sCR0);
}

static int arm_smmu_init(struct device_node *node,
			 enum arm_smmu_arch_version version,
			 enum arm_smmu_implementation model)
{
	int ret = 0, i;
	...
	smmu = zalloc(sizeof(*smmu));
	if (!smmu) {
		pr_err("%s: can't allocate device data\n",
			   __func__);
		ret = -ENOMEM;
		goto fail;
	}
	...
	if (of_get_bool(node, "dma-coheren"))
		smmu->features |= ARM_SMMU_FEAT_COHERENT_WALK;

	ret = of_translate_address(node, &pa, &size);
	if (ret) {
		pr_err("%s: can't map device regs\n", __func__);
		goto fail_free_smmu;
	}
	smmu->base = (void *)pa;
	smmu->reg_pa = pa;
	...
	pr_info("arm-smmu: phys=0x%"PRIPADDR" size=%"PRIPSIZE
		  "\n", pa, size);

	if (of_get_u32_array(node, "#global-interrupts",
				 &global_irqs, 1)) {
		pr_err("%s: can't find #global-intretupts "
			   "DT prop\n", __func__);
		ret = -ENODEV;
	...
	if (num_irqs < global_irqs) {
		pr_err("%s: number of global-intretupts "
	...
	pr_info("arm-smmu: num_irqs=%d num_global_irqs=%d\n",
		  num_irqs, global_irqs);

	if (!smmu->num_context_irqs) {
		pr_err("%s: need atleast one context irqs\n", __func__);
		ret = -ENODEV;
		goto fail_unmap_regs;
	}

	smmu->irqs = zalloc(sizeof(*smmu->irqs) * num_irqs);
	if (!smmu->irqs) {
	...
	for (i = 0; i < num_irqs; ++i) {
		uint32_t irq;
		ret = get_device_irq_index(node, &irq, &flags, i);
		if (ret) {
			pr_err("%s: failed to parse irq%d\n", __func__, i);
			ret = -ENODEV;
			goto fail_free_irqs;
		}
	...
	if (ret) {
		pr_err("%s: cfg_probe() failed\n", __func__);
		goto fail_free_irqs;
	}

	if (smmu->version == ARM_SMMU_V2 &&
	    smmu->num_context_banks != smmu->num_context_irqs) {
		pr_err("%s: found only %d context "
	...
	smmu->smrs = zalloc(smmu->num_mapping_groups * sizeof(*smmu->smrs));
	if (!smmu->smrs) {
		pr_err("%s: failed to alloc SMRs\n", __func__);
		ret = -ENOMEM;
		goto fail_free_irqs;
	}

	smmu->s2crs = zalloc(smmu->num_mapping_groups * sizeof(*smmu->s2crs));
	if (!smmu->s2crs) {
		pr_err("%s: failed to alloc S2CRs\n", __func__);
		ret = -ENOMEM;
		goto fail_free_smrs;
	}
	for (i = 0; i < smmu->num_mapping_groups; i++)
	...
	mutex_init(&smmu->stream_map_mutex, "smmu_mutex");

	smmu->cbs = zalloc(smmu->num_context_banks *sizeof(*smmu->cbs));
	if (!smmu->cbs) {
		pr_err("%s: failed to alloc CBs\n", __func__);
		ret = -ENOMEM;
		goto fail_free_s2crs;
	}
	atomic_set(&smmu->irptndx, 0);

	for (i = 0; i < smmu->num_global_irqs; ++i) {
		ret = request_irq(smmu->irqs[i], arm_smmu_global_fault,
				0, "arm-smmu-global-fault", smmu);
		if (ret) {
			pr_err("%s: failed to register global "
	...
	if (ret) {
		pr_err("%s: failed to register controller\n", __func__);
		goto fail_free_cbs;
	}

	spin_lock_irqsave(&smmu_devices_lock, flags);
	list_add_tail(&smmu->list, &smmu_devices);
	spin_unlock_irqrestore(&smmu_devices_lock, flags);

	pr_info("arm-smmu: ready!\n");
...
fail_free_irqs:
	free(smmu->irqs);
fail_unmap_regs:
	vmm_devtree_regunmap_release(node, (vir_addr_t)smmu->base, 0);
fail_free_smmu:
...
static int arm_smmu_v1_init(struct device_node *node)
{
	return arm_smmu_init(node, ARM_SMMU_V1, GENERIC_SMMU);
}

static int arm_smmu_v2_init(struct device_node *node)
{
	return arm_smmu_init(node, ARM_SMMU_V2, GENERIC_SMMU);
}

static int arm_smmu_401_init(struct device_node *node)
{
	return arm_smmu_init(node, ARM_SMMU_V1_64K, GENERIC_SMMU);
}

static int arm_smmu_500_init(struct device_node *node)
{
	return arm_smmu_init(node, ARM_SMMU_V2, ARM_MMU500);
}

static int cavium_smmu_v2_init(struct device_node *node)
{
	return arm_smmu_init(node, ARM_SMMU_V2, CAVIUM_SMMUV2);
}

CONTROLLER_DECLARE(smmu_v1, "arm,smmu-v1", arm_smmu_v1_init);
CONTROLLER_DECLARE(smmu_v2, "arm,smmu-v2", arm_smmu_v2_init);
CONTROLLER_DECLARE(smmu_400, "arm,mmu-400", arm_smmu_v1_init);
CONTROLLER_DECLARE(smmu_401, "arm,mmu-401", arm_smmu_401_init);
CONTROLLER_DECLARE(smmu_500, "arm,mmu-500", arm_smmu_500_init);
CONTROLLER_DECLARE(cavium_smmu_v2, "cavium,smmu-v2", cavium_smmu_v2_init);
